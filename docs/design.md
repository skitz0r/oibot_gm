...
- **Raid config** (`raids.yaml` defaults, guild overrides via `/gm config raid`, the Raids page, or plain text `raid_set`): `slots` (recurring times, e.g. `Tue 19:30`), `signup_lead_hours` (sheet opens this long before the slot; default 120, Barrow Deeps 48), `lock_hours_before` (24), `confirm_hours_before` (6), `fill_ask_hours` (4: silence on a fill DM counts as no), `weights` (seat-selection bonuses: `rank`, `main`, `sat_out`, `signup_order`). Slots before `first_open` never open.
