...
/** Fallback only: the live palette comes from /api/meta (`class_colours`, the Python side's dict) once it has loaded. */
...
export const classColour = (meta: { class_colours?: Record<string, string> } | null | undefined, cls: string): string | undefined => meta?.class_colours?.[cls] ?? CLASS_COLOURS[cls];
